...
import { Tables } from '@/src/supabase/types'; // Assuming Profile type is in src/types/index.ts or similar

export interface SearchedUserProfile extends Pick<Tables<'profiles'>, 'id' | 'username' | 'full_name' | 'avatar'> {
...
export const searchProfiles = async (
  ...
  return data as unknown as SearchedUserProfile[];
};
...
export const getFollowingList = async (userId: string): Promise<SearchedUserProfile[]> => {
  ...
  return data?.map(item => item.profile_being_followed).filter(Boolean) as unknown as SearchedUserProfile[] || [];
};
...
export const getFollowersList = async (userId: string): Promise<SearchedUserProfile[]> => {
  ...
  // The result is nested, so we need to map it
  return data?.map(item => item.follower_profile).filter(Boolean) as unknown as SearchedUserProfile[] || [];
};
