...
import { getSkillFormDataForSubmission, SequenceStep, useUserSkillFormStore } from '../stores/userSkillFormStore';
...
export default function CreateSkillScreen() {
  ...
  // Fetch all skills with React Query (replaces Zustand store)
  const { data: allSkills = [], isLoading: isLoadingAllSkills, error: errorAllSkills } = useQuery({
  ...
  useEffect(() => {
    if (editingUserSkill && categoriesLoaded && categoriesData && !selectedCategory) {
      const categoryToSet = (categoriesData as Tables<'categories'>[]).find((c: Tables<'categories'>) => c.id === editingUserSkill.skill.categoryId);
      if (categoryToSet) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['allSkills', userId] }); // Invalidate skills cache
        ...
        <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {isLoadingCategories ? (
            ...
            <CategoryPicker
              ...
              categories={categoriesData ? (categoriesData as Tables<'categories'>[]).map((c: Tables<'categories'>) => ({id: c.id, name: c.name})) : []}
            />
          )}
